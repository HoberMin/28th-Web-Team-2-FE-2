...
## 인증 작업이면
- **`auth-session` 스킬이 진실 소스** — 토큰 보관 위치·갱신 지점·캐싱 경계를 새로 설계하지 않는다
- 인증은 **위험 경로(L급)** — 계획 확인 후 진행하고, 푸시 전 code-reviewer 필수
- 미정(비회원 경계·신규회원 구분·refresh 회전)은 `농산물-문서/be-요청사항.md` 답변 대기. 우회 구현을 임의로 넣지 않는다
