...
2. **대비 검산** — 토큰을 바꾸면 대비가 바뀐다. 실측 사례:
   ...
   **용도는 우리 매핑이 틀렸는지 보는 신호다.** sync 전후로 대비가 떨어졌으면 값이 나빠진 게 아니라 **엉뚱한 토큰에 물렸을 가능성**을 먼저 의심한다 — 실제로 `content/rank` 건이 이 신호로 발견됐다(글자를 심볼 SVG 값에 맞추자 대비가 내려갔고, 그게 원래 둘이 어긋나 있었다는 증거였다).
   Figma가 미달 값을 준 경우 **원본을 유지**하고 임의로 색을 바꾸지 않는다. 단 그 수치를 **디자이너 피드백 문서에 항목으로 올리지 않는다** — 대비는 디자이너가 화면을 보고 정한 결과라 지적 대상이 아니다(`design-feedback` §선별 기준 2관문). 우리 매핑 오류로 판명되면 코드를 고치고, 아니면 거기서 끝낸다
