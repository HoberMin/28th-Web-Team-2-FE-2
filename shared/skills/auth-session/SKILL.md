...
- **인증 실패와 통신 실패를 구분한다.** 뭉치면 Spring이 잠깐 죽었을 때 로그인 사용자 전원의
  refreshToken까지 지워 재로그인시킨다. **세션 종료로 보는 건 401·403뿐**이고,
  5xx·404·429·타임아웃·이상한 응답은 쿠키를 건드리지 않고 통과시킨다(BFF는 503으로 응답).
  ⚠️ **이건 한 파일의 규칙이 아니라 재발급을 하는 모든 지점에 걸리는 규칙이다.**
  실제로 `proxy.ts`만 고치고 `app/api/auth/reissue`를 빠뜨려 같은 결함이 남았던 적이 있다.
  지금 재발급 지점은 둘이다(`proxy.ts` · `app/api/auth/reissue`) — 세 번째를 만들면 여기 적을 것.
- **"서버 사정"으로 분류한 실패에는 백오프를 건다.** 쿠키를 남긴 채 통과시키면 다음 요청도
  갱신 대상이라 **매 페이지뷰마다 실패할 재발급을 한 번씩 더 쏜다**(탭·프리페치만큼 배수).
  `proxy.ts`가 `mg_reissue_backoff` 쿠키로 60초 쉰다. 이건 안전망이고, 근본 해결은
  "Spring이 무효 refreshToken에 어떤 코드를 주는가"를 확인하는 것이다(BE 요청 A-3).
- **비-`ApiError` 예외는 삼키지 않고 재던진다.** 우리 버그(쿠키 쓰기 실패 등)를 503으로 포장하면
  클라이언트는 재시도가 답이라 믿고 원인은 로그에도 안 남는다.
- **실패는 `console.error`로 남긴다** — `kind`·`status`·`endpoint`만. 토큰은 절대 로그에 넣지 않는다
  (`ApiError`는 애초에 토큰을 담지 않는다).
...
- **개인화 상태만 바뀌는 뮤테이션은 공유 태그를 무효화하지 않는다.** 찜 토글이 그 예다 —
  찜 정보는 `no-store` 응답에만 있고 태그가 붙은 건 게스트 캐시뿐이라, 무효화하면
  **하트 한 번에 전 게스트 캐시가 날아간다.** 실제 데이터가 바뀌는 뮤테이션(제보 등)만 무효화한다.
- 무효화는 `app/_lib/api/tags.ts`의 `REVALIDATE_IMMEDIATELY`(`{ expire: 0 }`)를 쓴다.
  문자열 프로필(`"max"` 등)은 만료를 미래로 밀어 **즉시 무효화가 아니다**(`data-fetching`).
