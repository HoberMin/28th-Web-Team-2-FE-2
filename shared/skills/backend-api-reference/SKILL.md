...
```bash
curl -s https://api.marketgo.kro.kr/v3/api-docs > /tmp/api-docs.json
# 엔드포인트 목록
node -e "const d=require('/tmp/api-docs.json');for(const[p,o]of Object.entries(d.paths))for(const[m,v]of Object.entries(o))console.log(m.toUpperCase(),p,'|',v.summary||'')"
# 특정 엔드포인트 상세
node -e "console.log(JSON.stringify(require('/tmp/api-docs.json').paths['/api/v1/items'],null,2))"
# 스키마 하나
node -e "console.log(JSON.stringify(require('/tmp/api-docs.json').components.schemas.ItemResponse,null,2))"
```
