...
/** POST /api/v1/images 실시간 OpenAPI 제약. 클라이언트와 Server Action이 같은 값을 쓴다. */
export const MAX_UPLOAD_IMAGE_BYTES = 5 * 1024 * 1024;
export const ALLOWED_UPLOAD_IMAGE_TYPES = ["image/jpeg", "image/png"] as const;
...
export function validateUploadImage(file: {
  ...
  if (!ALLOWED_UPLOAD_IMAGE_TYPES.some((type) => type === file.type)) return "unsupportedType";
  return null;
}
