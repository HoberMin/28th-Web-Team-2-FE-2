...
export function PhoneFrame({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-screen justify-center bg-gray-100 md:items-center md:py-8">
      <div
        className="relative flex h-dvh w-full flex-col overflow-hidden bg-surface-primary md:h-[844px] md:rounded-[2rem] md:shadow-xl"
        style={{ maxWidth: 390 }}
      >
        {children}
      </div>
...
export function AppBar({ title, backHref, onBack, right }: AppBarProps) {
  const backClass =
    "absolute left-1 flex size-12 items-center justify-center rounded-full text-content-primary hover:bg-gray-100 [&_svg]:size-6";
  ...
  return (
    <header className="relative flex h-14 shrink-0 items-center justify-center px-2">
      ...
      {title && <h1 className="text-title-18-bold text-content-primary">{title}</h1>}
 ...
 * 스크롤은 이 영역 하나만 담당한다.
 *
 * ref — 단계형 화면(가게 모드·온보딩)이 다음 단계로 넘어갈 때 스크롤을 맨 위로 되돌리는 데 쓴다.
...
export function BottomBar({ children }: { children: ReactNode }) {
  return (
    <div
      className="shrink-0 border-t border-border-secondary bg-surface-primary px-4 pt-3"
