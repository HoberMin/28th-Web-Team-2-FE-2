  ...
  return (
    <section className="flex flex-col gap-4" aria-label={`${vegetableName} 시세`}>
      <div className="flex items-center gap-1.5">
        <h2 className="text-title-18-bold text-content-primary">{vegetableName} 시세</h2>
      ...
      >
        ...
        >
          ...
          <path
            ...
            vectorEffect="non-scaling-stroke"
          />
          <circle cx={active.x} cy={active.y} r="4.5" className="fill-orange-600" />
        </svg>
        ...
        >
          <p className="text-body-14-regular whitespace-nowrap text-content-inverse">
            {formatShortDate(activePoint.date)}
          </p>
          <p className="text-body-14-medium whitespace-nowrap text-content-inverse">
      ...
      <div className="relative h-[17px] text-body-14-medium text-content-primary">
        {labelIdx.map((li, i) => {
      ...
      <div className="flex items-center justify-between rounded-lg border border-border-primary px-4 py-3.5">
        <span className="text-body-14-medium text-content-primary">{PERIOD_LABEL[period]} 평균가</span>
        <span className="text-body-16-semibold text-content-primary">{formatWon(avg)}</span>
      </div>
    </section>
  );
}
