...
export function StoreMap({ storeName, district }: { storeName: string; district: string }) {
  ...
  return (
    <section aria-label="가게 위치" className="flex flex-col gap-2">
      <h2 className="text-body-16-bold text-content-primary">위치</h2>
      ...
      <div
        ref={containerRef}
        aria-hidden="true"
        className="flex h-40 w-full items-center justify-center overflow-hidden rounded-2xl bg-gray-100"
      >
        {status === "failed" && (
          <span className="text-caption-12-regular text-content-secondary">지도를 표시할 수 없어요</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl bg-gray-100 px-4 py-3">
        ...
        <span className="flex min-w-0 flex-1 flex-col">
          <span className="truncate text-body-14-medium text-content-primary">{location.address}</span>
