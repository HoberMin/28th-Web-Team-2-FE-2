    ...
    <div className={className}>
      ...
      <div aria-live="polite" aria-atomic="true">
        <button
          ...
          className="flex h-12 w-full items-center gap-2 rounded-2xl bg-gray-100 px-4 text-left text-body-14-medium text-content-primary active:bg-gray-200 disabled:opacity-60"
        >
          <IconLocationpinLine className="size-5 text-orange-700" aria-hidden="true" />
          {locateStatus === "locating" ? "위치 확인 중…" : "현재 위치로 찾기"}
        </button>
        {locateStatus === "failed" && (
          <p className="mt-1 px-1 text-caption-12-regular text-content-secondary">
      ...
      {!searching && (
        <p className="mt-6 text-body-14-regular text-content-secondary">
      ...
      >
        {regions.length === 0 ? (
          <li className="py-12 text-center text-body-14-regular text-content-secondary">
            검색 결과가 없어요
          </li>
        ) : (
          regions.map((region) => (
            <li key={region.id}>
              <button
                ...
                className="flex h-12 w-full items-center text-left active:bg-gray-100"
              >
                <span className="text-body-16-regular text-content-primary">{region.label}</span>
              </button>
            </li>
          ))
        )}
