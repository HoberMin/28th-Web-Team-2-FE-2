              ...
              results.map((veg) => (
                <li key={veg.id}>
                  <button
                    type="button"
                    onClick={() => choose(veg)}
                    className="flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-left active:bg-gray-100"
                  >
                    <span className="text-xl" aria-hidden="true">
                      {veg.emoji}
                    </span>
                    <span className="text-body-16-medium text-content-primary">{veg.name}</span>
                  </button>
                </li>
              ))
            )}
