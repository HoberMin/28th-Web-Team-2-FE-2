  ...
  return (
    <section aria-label="온라인 판매가 비교" className="flex flex-col gap-3">
      <div className="flex items-baseline gap-1.5">
        <h3 className="text-body-16-semibold text-content-primary">온라인가 비교</h3>
        {set.hasEstimated && (
          <span className="text-caption-12-regular text-content-secondary">예시 데이터 포함</span>
        )}
      </div>

      {/* 기준선 — 오프라인 값(동네 제보가 우선, 없으면 오늘 시세). 비교 대상이 무엇인지
          목록 위에 한 줄로 못 박아 둔다(막대 없이도 이 줄이 있으면 "동네보다 싼가"가 읽힌다). */}
      <div className="flex items-center justify-between rounded-xl bg-gray-100 px-3 py-2 text-caption-12-regular text-content-secondary">
          ...
          return (
            <li key={p.mall}>
              <a
                ...
                className="flex items-center gap-2 border-b border-border-secondary py-3 last:border-b-0 active:bg-gray-100"
              >
                <span className="flex min-w-0 flex-1 flex-col">
                  <span className="truncate text-body-14-medium text-content-primary">{p.mall}</span>
                ...
                <span className="flex shrink-0 flex-col items-end">
                  ...
                  <span
                    className={`text-caption-12-regular tabular-nums ${
                      cheaper ? "text-green-600" : "text-content-secondary"
                    }`}
                  >
                    {cheaper ? "기준보다 싸요" : "기준보다 비싸요"}
                  </span>
                </span>
                <span className="shrink-0 text-content-secondary [&_svg]:size-4" aria-hidden="true">
