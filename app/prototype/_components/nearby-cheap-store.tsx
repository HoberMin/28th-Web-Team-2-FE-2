  ...
  return (
    <section aria-label="이 가게 어때요?" className="flex flex-col gap-3">
      <div className="flex flex-col gap-0.5">
        <h2 className="text-body-16-bold text-content-primary">이 가게 어때요?</h2>
      ...
      <Link
        ...
        className="flex flex-col gap-3 rounded-2xl bg-orange-50 px-4 py-4 active:bg-orange-100"
      >
        <span className="flex items-center gap-2">
          <span className="min-w-0 flex-1 truncate text-body-16-semibold text-content-primary">
            {winner.summary.name}
          </span>
          <span className="shrink-0 text-caption-12-regular tabular-nums text-content-secondary">
          ...
          <span className="flex items-center gap-1.5">
            {cheaperItems.slice(0, THUMB_LIMIT).map((item) => (
              <span key={item.vegetableId} className="flex flex-col items-center gap-0.5">
                ...
                <span className="text-caption-12-regular text-content-primary">{item.name}</span>
              </span>
            ))}
            {cheaperItems.length > THUMB_LIMIT && (
              <span className="self-center text-caption-12-regular tabular-nums text-content-secondary">
                +{cheaperItems.length - THUMB_LIMIT}
              </span>
            )}
          </span>
