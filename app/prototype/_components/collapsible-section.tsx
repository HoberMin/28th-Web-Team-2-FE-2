...
export function CollapsibleSection({
  ...
  return (
    <section aria-label={title} className="flex flex-col gap-3 px-4 pt-6">
      <h2 className="text-title-18-bold text-content-primary">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
        ...
        >
          <span className="flex items-baseline gap-2">
            {title}
            {note && <span className="text-caption-12-regular text-content-secondary">{note}</span>}
          </span>
          <span
            className={`shrink-0 text-content-secondary transition-transform [&_svg]:size-5 ${
              open ? "rotate-180" : ""
            }`}
            aria-hidden="true"
          >
