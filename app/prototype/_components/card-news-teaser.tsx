    ...
    <section aria-label="이번 주 시세 이야기" className="flex flex-col gap-2">
          ...
          return (
            <li key={news.id} className="w-full shrink-0 snap-start">
              ...
              <Link
                ...
                className="flex items-center justify-between gap-3 rounded-2xl bg-orange-50 px-4 py-3.5 active:bg-orange-100"
              >
                <span className="flex min-w-0 flex-col">
                  <span className="text-caption-12-regular text-content-secondary">이번 주 시세 이야기</span>
                  <span className="truncate text-body-14-medium text-content-primary">
                    {veg?.emoji} {news.title}
                  </span>
                </span>
                <span aria-hidden="true" className="shrink-0 text-body-14-medium text-content-secondary">
                  더보기
                </span>
              </Link>
            </li>
      ...
      <div className="flex items-center justify-center gap-1.5" aria-hidden="true">
        {CARD_NEWS.map((news, i) => (
          <span
            key={news.id}
            className={`size-1.5 rounded-full ${i === active ? "bg-orange-600" : "bg-gray-200"}`}
          />
        ))}
      </div>
    </section>
