  ...
  return (
    <section aria-label="우리 동네 최저가 야채" className="flex flex-col gap-3">
      <div className="flex items-center justify-between gap-2">
        <h2 className="shrink-0 text-body-16-bold text-content-primary">우리 동네 최저가 야채</h2>
        <div className="flex min-w-0 items-center gap-1">
          <span className="truncate text-caption-12-regular text-content-secondary">
            일주일 동안 이웃이 제보한 가격
          </span>
          ...
          <Link
            ...
            className="flex size-8 shrink-0 items-center justify-center rounded-full text-content-secondary active:bg-gray-100 [&_svg]:size-5"
          >
        ...
        {list.map((item, i) => (
          <li key={item.vegetableId}>
            <Link
              ...
              className="flex h-14 items-center gap-3 border-b border-border-secondary last:border-b-0 active:bg-gray-100"
            >
              <span className="w-5 shrink-0 text-body-16-semibold tabular-nums text-content-secondary">
                {i + 1}
              </span>
              <VegetableThumb image={item.image} emoji={item.emoji} size="sm" />
              <span className="flex min-w-0 flex-1 flex-col">
                <span className="truncate text-body-14-medium text-content-primary">{item.name}</span>
                {item.place && (
                  // 가게명 앞 아이콘 — 두 번째 줄이 품목 설명이 아니라 "어느 가게"인지 바로 알게 한다.
                  <span className="flex min-w-0 items-center gap-1 text-caption-12-regular text-content-secondary">
              ...
              <span className="flex shrink-0 flex-col items-end">
                <span className="text-body-16-semibold tabular-nums text-content-primary">
                  {formatWon(item.price)}
                </span>
                <span className="text-caption-12-regular tabular-nums text-green-600">
      ...
      {(canShowMore || expanded) && (
        <button
          ...
          className="h-11 w-full rounded-xl bg-gray-100 text-body-14-medium text-content-primary active:bg-gray-200"
        >
          {expanded ? "닫기" : "더보기"}
        </button>
      )}
