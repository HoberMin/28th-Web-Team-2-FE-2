  ...
  return (
    <div className="flex flex-col gap-4 px-4 pt-1 pb-8">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <BottomSheetRoot open={sortSheetOpen} onOpenChange={setSortSheetOpen}>
            <BottomSheetTrigger asChild>
              <button
                type="button"
                className="flex shrink-0 items-center gap-1 rounded-lg border border-border-secondary px-3 py-1.5 text-caption-12-medium text-content-primary active:bg-gray-100"
              >
                정렬 · {sortLabel}
                <span className="text-content-secondary [&_svg]:size-3.5" aria-hidden="true">
                ...
                {SORT_OPTIONS.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between rounded-xl px-3 py-3 active:bg-gray-100 ${
                        selected ? "text-body-16-semibold text-orange-700" : "text-body-16-regular text-content-primary"
                      }`}
                    >
                      {option.label}
                      {selected && (
                        <span className="text-orange-700 [&_svg]:size-5" aria-hidden="true">
                          <IconCheckmarkLine />
                        </span>
                      )}
                    </button>
          ...
          <span
            role="status"
            aria-live="polite"
            className="truncate text-caption-12-regular tabular-nums text-content-secondary"
          >
            {metaText}
          </span>
        </div>
        ...
        {list.length === 0 ? (
          <p className="py-12 text-center text-body-14-regular text-content-secondary">
          ...
          <>
            ...
            {canShowMore && (
              <button
                ...
                className="flex h-11 w-full items-center justify-center rounded-xl bg-gray-100 text-body-14-medium text-content-primary active:bg-gray-200"
              >
