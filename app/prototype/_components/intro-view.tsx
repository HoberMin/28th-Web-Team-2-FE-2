  ...
  return (
    <PhoneFrame>
      <Scroll className="px-6 pt-14">
        ...
        <h1 className="mt-8 text-center text-title-24-medium font-bold leading-tight text-content-primary">
          우리 동네 야채,
          <br />
          오늘 얼마인지 이웃이 알려줘요
        </h1>
          ...
          {VALUES.map((value) => {
            ...
            return (
              <li key={value.title} className="flex items-center gap-3">
                <span className="flex size-11 shrink-0 items-center justify-center rounded-full bg-orange-50">
                  {veg?.image ? (
                    <Image
                      src={veg.image}
                      alt=""
                ...
                <span className="flex min-w-0 flex-col">
                  <span className="text-body-16-semibold text-content-primary">{value.title}</span>
                  <span className="text-caption-12-regular text-content-secondary">{value.body}</span>
                </span>
              </li>
            );
          })}
      ...
      <BottomBar>
        ...
        {error && (
          <p role="alert" className="mt-2 text-center text-caption-12-regular text-red-600">
            {error}
          </p>
        )}
        ...
        <button
          ...
          disabled={pending}
          className="mt-1 h-12 w-full text-body-14-regular text-content-secondary underline disabled:opacity-60"
        >
          먼저 둘러볼게요
        </button>

        <p className="mt-1 text-center text-caption-12-regular text-content-secondary">
          시작하면{" "}
