  ...
  return (
    <ul className="flex flex-col gap-2">
      {favoriteStores.map((name) => (
        <li key={name} className="flex items-center gap-2 rounded-2xl bg-gray-100 px-4 py-3">
          <Link
            href={`/prototype/store/${encodeURIComponent(name)}`}
            className="min-w-0 flex-1 rounded-xl active:opacity-70"
          >
            <span className="truncate text-body-14-medium text-content-primary">{name}</span>
          </Link>
          <button
            type="button"
