...
// 하이드레이션 감지용 스냅샷 — 구독할 외부 소스가 없으므로 subscribe는 no-op이다.
// 모듈 상수로 두어 매 렌더 새 함수가 생기지 않게 한다(useSyncExternalStore가 재구독하지 않도록).
...
export function StoreDetail({
  ...
  // 로딩 스켈레톤만 보여준다.
  // useEffect + setState 대신 useSyncExternalStore를 쓴다 — 서버 스냅샷(false)과 클라 스냅샷(true)이
  // 다르다는 사실 자체를 React에 알려주는 방식이라 추가 렌더가 생기지 않는다(effect 내 동기 setState 금지).
  const hydrated = useSyncExternalStore(EMPTY_SUBSCRIBE, getHydratedSnapshot, getServerSnapshot);
      ...
      <PhoneFrame>
        ...
        <Scroll className="px-4 pb-8">
          <div className="flex flex-col items-center gap-2 pt-16 text-center">
            <p className="text-body-16-semibold text-content-primary">찾을 수 없는 가게예요</p>
            <p className="text-body-14-regular text-content-secondary">
              주소가 바뀌었거나 없는 가게일 수 있어요.
            </p>
            <Link
              href="/prototype/stores"
              className="mt-4 flex min-h-11 items-center justify-center rounded-lg bg-gray-100 px-4 py-2.5 text-body-14-medium text-content-primary active:bg-gray-200"
            >
              가게 둘러보기
            </Link>
          </div>
    ...
    <PhoneFrame>
      ...
      <Scroll className="px-4 pb-8">
        ...
        {items.length === 0 ? (
          <div className="flex flex-col gap-6 pt-2">
            <div className="flex flex-col items-center gap-2 px-4 text-center">
              <p className="text-body-16-semibold text-content-primary">아직 이 가게 제보가 없어요</p>
              <p className="text-body-14-regular text-content-secondary">
                다녀오셨다면 가격을 남겨주세요. 이웃이 헛걸음하지 않아요.
              </p>
            </div>
            {hasReviews && (
              <p className="text-center text-caption-12-regular text-content-secondary">
          ...
          <div className="flex flex-col gap-6 pt-2">
            ...
            <section
              aria-label="가게 요약"
              className="flex flex-col gap-2 rounded-2xl bg-gray-100 px-5 py-4"
            >
              {cheaper.length > 0 ? (
                <p className="text-body-16-bold text-green-600">
                  {cheaper.length}개 품목이 {BASELINE_LABEL}보다 싸요
                </p>
              ) : (
                <p className="text-body-16-bold text-content-primary">이웃 제보 {items.length}개 품목</p>
              )}
            ...
            <section aria-label="품목별 제보가" className="flex flex-col gap-2">
              ...
              <h2 className="text-body-14-medium text-content-primary">이 가게에 제보된 가격</h2>
              ...
              {ordered.length === 0 && (
                <p className="py-8 text-center text-body-14-regular text-content-secondary">
                  {priceTab === "cheap"
                  ...
                  return (
                    <li key={i.vegetableId}>
                      <Link
                        ...
                        className="flex items-center gap-3 rounded-2xl bg-gray-100 px-4 py-3 active:bg-gray-200"
                      >
                        <VegetableThumb image={i.image} emoji={i.emoji} size="md" />
                        <span className="flex min-w-0 flex-1 flex-col gap-0.5">
                          <span className="flex items-center gap-1.5">
                            <span className="text-body-16-semibold text-content-primary">{i.name}</span>
                            {i.crossChecks >= 2 && (
                              // 무채색 배지 — 초록은 "싸다" 전용이라 교차검증(신뢰 신호)엔 안 쓴다(#13)
                              <span className="rounded-md bg-surface-primary px-1.5 py-0.5 text-caption-12-regular text-content-secondary">
                          ...
                          <span className="flex items-center gap-1.5">
                            ...
                            {i.outlier && (
                              <span className="text-caption-12-regular text-orange-700">
                        ...
                        <span className="flex shrink-0 flex-col items-end">
                          <span className="text-body-14-medium tabular-nums text-content-primary">
                            ...
                            <span className="text-content-secondary"> /{i.unit}</span>
                          </span>
        ...
        <section aria-label="동네 댓글" className="flex flex-col gap-3 pt-6">
          <h2 className="text-body-16-bold text-content-primary">동네 댓글</h2>
          <CommentList
            storeName={storeName}
            todayIso={todayIso}
            hydrated={hydrated}
