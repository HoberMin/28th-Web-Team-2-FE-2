...
export function LatestReportPrice({ vegetableId }: { vegetableId: string }) {
    ...
    if (nearbyLatest) {
      return (
        <span className="flex flex-col items-end">
          <span className="text-head1-20 tabular-nums text-fg-neutral">
            {formatNumber(nearbyLatest.pricePerKg)}원
          </span>
          <span className="text-caption-12-regular text-fg-neutral-muted">{nearby.district} 제보 기준</span>
        </span>
      );
    }
  ...
  return <span className="text-head1-20 tabular-nums text-fg-neutral">{formatNumber(latest.pricePerKg)}원</span>;
}
    ...
    <div className="flex flex-col gap-3">
      ...
      <ul className="flex flex-col gap-2">
        {visible.map((r) => {
          ...
          return (
            <li
              key={r.id}
              className={`flex flex-col gap-1.5 rounded-2xl bg-bg-neutral-weak px-4 py-3 ${
                outlier ? "opacity-60" : ""
              }`}
            >
              {/* 윗줄 — 가게명 + 가격 */}
              <div className="flex items-center justify-between gap-2">
                {r.place ? (
                  <Link
                    ...
                    className="min-w-0 truncate text-body-16-semibold text-fg-neutral underline decoration-dotted"
                  >
                    {r.place}
                  </Link>
                ) : (
                  <span className="text-body-16-semibold text-fg-neutral-muted">가게 미기재</span>
                )}
                <span className="shrink-0 text-body-16-semibold tabular-nums text-fg-neutral">
                  {formatNumber(r.pricePerKg)}원
                  <span className="text-body-14-regular text-fg-neutral-muted"> /{unit}</span>
                </span>
              </div>

              {/* 아랫줄 — 제보 시점·교차검증 + 시세 대비 차이(가격과 같은 급으로) */}
              <div className="flex items-center justify-between gap-2">
                <span className="flex min-w-0 flex-wrap items-center gap-1 text-caption-12-regular text-fg-neutral-muted">
                ...
                {outlier ? (
                  <span className="shrink-0 rounded bg-bg-layer-default px-1.5 py-0.5 text-caption-12-regular text-fg-neutral-muted">
                    확인 필요
                  </span>
                ) : (
                  <span
                    className={`flex shrink-0 items-center gap-0.5 text-body-14-medium tabular-nums ${
                      diff === 0 ? "text-fg-neutral-muted" : cheaper ? "text-fg-positive" : "text-fg-critical"
                    }`}
                  >
      ...
      {ordered.length > PREVIEW_COUNT && (
        <button
          ...
          className="mt-1 flex min-h-11 w-full items-center justify-center rounded-lg bg-bg-neutral-weak py-3.5 text-center text-body-14-medium text-fg-neutral active:bg-bg-neutral-weak-pressed"
        >
