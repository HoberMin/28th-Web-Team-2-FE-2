  ...
  if (!open) {
    return (
      <button
        ...
        onClick={() => toggle(true)}
        aria-label="야채 검색"
        className="flex size-10 items-center justify-center rounded-full text-fg-neutral active:bg-bg-neutral-weak [&_svg]:size-6"
      >
    ...
    <div className="flex flex-1 flex-col gap-2">
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          onClick={close}
          aria-label="검색 닫기"
          className="flex size-10 shrink-0 items-center justify-center rounded-full text-fg-neutral-muted active:bg-bg-neutral-weak [&_svg]:size-5"
        >
          <IconXmarkLine />
        </button>
      </div>
      ...
      {keyword === "" ? (
        <p className="px-1 text-caption-12-regular text-fg-neutral-muted">
          찾는 야채 이름을 입력해 보세요
        </p>
      ) : results.length === 0 ? (
        <p role="status" className="px-1 text-caption-12-regular text-fg-neutral-muted">
          ‘{keyword}’ 검색 결과가 없어요
        </p>
      ) : (
          ...
          {results.map((veg) => (
            <li key={veg.id}>
              <Link
                ...
                onClick={close}
                className="flex h-12 items-center gap-2 border-b border-bg-neutral-weak last:border-b-0 active:bg-bg-neutral-weak"
              >
                <VegetableThumb image={veg.image} emoji={veg.emoji} size="sm" />
                <span className="truncate text-body-14-medium text-fg-neutral">{veg.name}</span>
              </Link>
            </li>
          ))}
        </ul>
