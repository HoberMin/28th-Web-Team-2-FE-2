...
export function WeeklyCalendar({ todayIso }: { todayIso: string }) {
  ...
  return (
    <section aria-label="이번 주 제보" className="flex flex-col gap-3 rounded-2xl bg-bg-neutral-weak px-4 py-4">
      <div className="flex items-baseline justify-between">
        <h2 className="text-body-16-semibold text-fg-neutral">이번 주 제보</h2>
        ...
        {keys.map((key, i) => {
          ...
          return (
            <li key={key} className="flex flex-col items-center gap-1">
              <span className="text-caption-12-regular text-fg-neutral-muted">{WEEKDAYS[i]}</span>
              <span
                className={`flex size-9 items-center justify-center rounded-full ${
                  veg ? "bg-bg-brand-weak" : "bg-bg-layer-default"
                } ${isToday ? "ring-1 ring-fg-brand-contrast" : ""}`}
              >
                {veg ? (
